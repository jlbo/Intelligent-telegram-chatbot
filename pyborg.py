...
class pyborg(object):
 ...
 !version, !words, !limit, !save, !owner"
	...
	def process_msg(self, io_module, body, replyrate, learn, owner, bot, update):
		"""
		...
		# Parse commands
		if body[0] == "!":
			self.do_commands(io_module, body, owner, bot, update)
			return
		...
		if randint(0, 99) < replyrate:
			...
			if len(message.split()) == 1:
				io_module.output(message, bot, update)
				return
			# empty. do not output
			if message == "":
				print('empty word output')
				return
			# else output
			io_module.output(message, bot, update)

	def do_commands(self, io_module, body, owner, bot, update):
		"""
		...
		if owner == 1:
			...
			#Remove rares words
				...
				if c_max < 1:
					#io_module.output(str(compteur)+" words to remove", bot, update)
					io_module.output("%s words to remove" %compteur, bot, update)
					return
			...
			# Print contexts [flooding...:-]
				...
				if context == "":
					return
				io_module.output("Contexts containing \""+context+"\":", bot, update)
				# Build context list
				# Pad it
				context = " "+context+" "
				c = []
				...
				while x < 5:
					if x < len(c):
						io_module.output(c[x], bot, update)
					x += 1
				if len(c) == 5:
					return
				if len(c) > 10:
					io_module.output("...("+repr(len(c)-10)+" skipped)...", bot, update)
				x = len(c) - 5
				if x < 5:
					x = 5
				while x < len(c):
		...
		if msg != "":	
			io_module.output(msg, bot, update)
